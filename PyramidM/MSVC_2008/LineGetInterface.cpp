#include "stdafx.h"
#include <MSVC_2008\LineGetInterface.h>


LineGetInterface::LineGetInterface(GisView *pt) : GisInputInterface (pt, true)
{
...
LineGetInterface::~LineGetInterface()
{
	DELETE_GISDATA(m_gisView, m_pLassoLayer);
}

/** обработчик события нажатия клавиш манипулятора мышь */
RetTypeRedraw LineGetInterface::mousePressEvent(const Mouse& mouse, ImageDevice *dev)
{
	PosXY pointXY;
...
RetTypeRedraw LineGetInterface::mouseMoveEvent(const Mouse& mouse, ImageDevice *dev)
{
	if (isPressOnce)
	{
		PosXY pointXY;
...
RetTypeRedraw LineGetInterface::detach() 
{ 
	DELETE_GISDATA(m_gisView, m_pLassoLayer);
	return topRedraw; 
}